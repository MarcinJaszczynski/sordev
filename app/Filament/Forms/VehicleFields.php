...
final class VehicleFields
{
    ...
    public static function schema(
        ...
        $fields = [
            ...
            Forms\Components\TextInput::make('manufacture_year')
                ...
                ->nullable()
                ->helperText('Rok produkcji autokaru.'),
        ...
        if ($includeAdHoc) {
            $fields[] = Forms\Components\Toggle::make('is_ad_hoc')
                ->label('Pojazd ad-hoc')
        ...
        $fields[] = Forms\Components\CheckboxList::make('equipment')
            ->label('Wyposażenie')
     ...
     * Kontrahent ustawiany z kontekstu (Transport / RM) — nie w tym schemacie.
     *
     * @return array<int, Forms\Components\Component>
     */
    ...
    {
        return [
            ...self::schema(includeContractor: false, includeMedia: false, includeAdHoc: false),
            ...self::mediaAndNotesFields(),
        ];
    }

    /**
     * Pełny formularz edycji z kontekstu imprezy (bez zmiany przewoźnika w selectcie —
     * podpięcie idzie z firmy transportowej / karty kontrahenta).
     *
     * @return array<int, Forms\Components\Component>
     */
    public static function editFromTransportSchema(): array
    {
        return [
            Forms\Components\Section::make('Dane pojazdu')
                ->columns(2)
                ->schema(self::schema(includeContractor: false, includeMedia: false)),
            Forms\Components\Section::make('Zdjęcia, pliki i uwagi')
                ->schema(self::mediaAndNotesFields()),
        ];
    }
    ...
    public static function attributesFromForm(array $data): array
    {
        return [
            ...
            'manufacture_year' => filled($data['manufacture_year'] ?? null)
                ? (int) $data['manufacture_year']
                : null,
            'registration_number' => filled($data['registration_number'] ?? null)
                ? mb_strtoupper(trim((string) $data['registration_number']))
                : null,
            ...
            'status' => $data['status'] ?? VehicleStatus::Active->value,
            'is_ad_hoc' => (bool) ($data['is_ad_hoc'] ?? false),
        ];
    }
    ...
    /**
     * Pełny payload create/update z formularza (dane + media).
     *
     * @param  array<string, mixed>  $data
     * @return array<string, mixed>
     */
    public static function payloadFromForm(array $data): array
    {
        return [
            ...self::attributesFromForm($data),
            ...self::mediaPayloadFromForm($data),
        ];
    }
