...
class EventHotelPlanning extends Page
{
    ...
    {
        return app(StickyNoteService::class)->countFor($this->record, StickyNoteCategory::HOTEL);
    }

    /**
     * Przełącza na zakładkę z notatkami i scrolluje do sekcji karteczek.
     */
    public function goToHotelStickyNotes(): void
    {
        $this->setHotelTab('services');
        $this->js(<<<'JS'
            setTimeout(() => {
                document.getElementById('hotel-sticky-notes')?.scrollIntoView({ behavior: 'smooth', block: 'start' });
            }, 50);
        JS);
    }
