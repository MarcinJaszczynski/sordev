        ...
        return $form
            ->schema([
                Forms\Components\Section::make('Podgląd portalu klienta')
                    ->icon('heroicon-o-eye')
                    ->description('Otwórz hub tej imprezy albo podgląd konkretnego konta (uczestnik/opiekun) z listy dostępów poniżej.')
                    ->schema([
                        Forms\Components\Placeholder::make('client_preview_link')
                            ...
                            ->content(fn (): \Illuminate\Support\HtmlString => new \Illuminate\Support\HtmlString(
                                '<a href="'.e(\App\Filament\Client\Resources\ClientEventResource::getUrl('view', ['record' => $this->getRecord()->id], panel: 'portal').'?preview=1').'" target="_blank" rel="noopener" class="inline-flex items-center gap-2 rounded-lg bg-blue-600 px-4 py-2 text-sm font-semibold text-white hover:bg-blue-700">'
                                .'<span>Podgląd hubu tej imprezy</span>'
                                .'</a>'
                                .'<p class="mt-2 text-xs text-gray-500">Aby zobaczyć płatności / uczestników jak klient, użyj „Podgląd jako…” przy konkretnym dostępie.</p>'
                            )),
                    ]),
