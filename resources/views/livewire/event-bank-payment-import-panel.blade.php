<div class="mb-8 space-y-6">
    <form wire:submit.prevent="runImport" class="space-y-4">
        <x-filament::section>
            ...
            <div>
                <label class="fi-fo-field-wrp-label inline-flex items-center gap-x-3" for="eventBankCsvFile">
    ...
    @if ($hiddenOtherEventsCount > 0)
        <p class="text-sm text-warning-600">
            {{ $hiddenOtherEventsCount }} {{ $hiddenOtherEventsCount === 1 ? 'wpłata dotyczy' : 'wpłat dotyczy' }}
            innych imprez i nie jest wyświetlana w tym widoku.
        </p>
    @endif

    @if ($lastImportSummary)
        <x-filament::section heading="Podsumowanie wczytania">
            <dl class="grid grid-cols-2 gap-4 text-sm sm:grid-cols-4">
                <div>
                    <dt class="font-medium text-gray-500">Wpływy (ta impreza)</dt>
                    <dd class="text-lg font-semibold">{{ count($previewLines) }}</dd>
                </div>
                <div>
                    <dt class="font-medium text-gray-500">Dopasowane</dt>
                    <dd class="text-lg font-semibold text-success-600">
                        {{ collect($previewLines)->where('match_status', '!=', 'unmatched')->count() }}
                    </dd>
                </div>
                <div>
                ...
                <div>
                    <dt class="font-medium text-gray-500">Inne imprezy</dt>
                    <dd class="text-lg font-semibold">{{ $hiddenOtherEventsCount }}</dd>
                </div>
            </dl>
        </x-filament::section>
    @endif
    ...
    @if ($previewLines !== [])
        <x-filament::section heading="Podgląd wpłat">
    ...
    @if ($lastApplySummary && ! empty($lastApplySummary['errors']))
        <x-filament::section heading="Ostrzeżenia przy księgowaniu">
