<div class="space-y-4">
    ...
    @if(! $readOnly)
        <div class="grid gap-3 rounded-xl border border-gray-200 bg-gray-50 p-4 dark:border-gray-700 dark:bg-gray-900/40 md:grid-cols-2">
            ...
            <div>
                <label class="mb-1 block text-sm font-medium">Data i godzina</label>
            ...
            <div class="md:col-span-2">
                <x-filament::button wire:click="addCollection" size="sm" icon="heroicon-o-plus">
                    Dodaj zbiórkę
                </x-filament::button>
            </div>
        </div>
    ...
    <div class="divide-y rounded-xl border border-gray-200 dark:border-gray-700">
            ...
            <div class="flex flex-col gap-2 px-4 py-3 text-sm sm:flex-row sm:items-center sm:justify-between">
                <div>
                    <div class="font-medium text-gray-900 dark:text-gray-100">
                        {{ $collection->title }}
                        <span class="ml-2 text-gray-500">· {{ $money((float) $collection->amount, $collection->currency) }}</span>
                    </div>
                    <div class="text-gray-600 dark:text-gray-400">
                        {{ $collection->collected_at?->format('d.m.Y H:i') ?? '—' }}
                ...
                @if(! $readOnly)
                    <div class="flex flex-wrap items-center gap-2">
                        ...
                        @elseif($collection->status === 'handed_to_office')
        ...
        @empty
            <div class="px-4 py-6 text-center text-sm text-gray-500">Brak zarejestrowanych zbiórek w autokarze.</div>
        @endforelse
    </div>
</div>
