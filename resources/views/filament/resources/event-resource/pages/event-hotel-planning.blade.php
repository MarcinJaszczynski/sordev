<x-filament-panels::page>
    @php
        ...
        $bedCapacity = \App\Support\EventHotelBedCapacity::analyzeOccupancy($occupancy);
        $bedCapacityWarningsHtml = \App\Support\EventHotelBedCapacity::warningsHtmlFromAnalysis($bedCapacity);
        ...
        $hotelNotesCount = $this->hotelStickyNotesCount();
    @endphp
    ...
    <div class="hotel-page-root space-y-0">
        ...
        <div class="hotel-status-bar hotel-status-bar--compact">
            ...
            @if ($hotelNotesCount > 0)
                <button
                    type="button"
                    class="hotel-notes-jump"
                    wire:click="goToHotelStickyNotes"
                    title="Przejdź do notatek hotelowych"
                >
                    📝 {{ \App\Services\StickyNoteService::countLabel($hotelNotesCount) }}
                </button>
            @endif
                        ...
                        <div class="hotel-night-chip hotel-night-chip--inline" title="{{ collect([
                            ...
                            @if ($nightHotel['name'])
                                @if ($nightHotel['edit_url'])
                                    <a href="{{ $nightHotel['edit_url'] }}" class="hotel-night-chip-link">{{ $nightHotel['name'] }}</a>
                                @else
        ...
        <div class="hotel-stat-grid">
            ...
            <div @class(['hotel-stat-card', 'hotel-stat-card--danger' => $bedCapacity['has_deficiency']])>
                <p class="hotel-stat-label">Miejsca / noc (plan → potrzeba)</p>
                <p class="hotel-stat-value">
                    @if ($bedCapacity['min_beds'] !== null)
                        {{ $bedCapacity['min_beds'] }} / {{ $bedCapacity['required'] }}
                    @else
        ...
        @if ($bedCapacityWarningsHtml)
            <div class="mb-3">
                {!! $bedCapacityWarningsHtml !!}
            </div>
        @endif
        ...
        <div @class(['hidden' => $hotelTab !== 'services'])>
            <div class="hotel-services-columns">
                ...
                <div class="hotel-services-col" id="hotel-sticky-notes">
                    <div class="hotel-card">
                        <div class="hotel-card-header">
