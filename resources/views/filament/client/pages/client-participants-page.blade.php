<x-filament-panels::page>
    ...
    <div class="mb-4 client-portal-section">
        <div class="flex flex-wrap items-center justify-between gap-3">
            <div>
                <p class="text-sm font-semibold text-slate-900">Kompletność listy: {{ $c['completeness_pct'] }}%</p>
        ...
        <div class="mt-3 h-2 overflow-hidden rounded-full bg-slate-100">
            <div class="h-full rounded-full bg-[#0663fc]" style="width: {{ max(0, min(100, $c['completeness_pct'])) }}%"></div>
        </div>
    </div>
    ...
    <div class="grid gap-4 lg:grid-cols-[minmax(0,1fr)_18rem]">
        <section class="client-portal-section overflow-hidden !p-0">
            <header class="border-b border-slate-100 bg-slate-50 px-4 py-3">
                <h3 class="text-sm font-semibold text-slate-900">Lista uczestników</h3>
            </header>
            @if ($this->participants === [])
                <p class="px-4 py-4 text-sm text-slate-500">Brak uczestników. Dodaj pierwszą osobę w formularzu obok.</p>
            @else
                                ...
                                <tr wire:key="client-participant-{{ $participant->id }}">
                                    ...
                                    <td class="px-4 py-2">
                                        ...
                                        @if($hasDiet && $diet['enabled'] && $diet['per_person_pln'] > 0)
                                            <div class="text-[11px] font-medium text-[#0663fc]">
                                    ...
                                    <td class="px-4 py-2 text-right whitespace-nowrap">
                                        <button type="button" class="text-[#0663fc] hover:underline" wire:click="startEdit({{ $participant->id }})">Edytuj</button>
                                        <button type="button" class="ml-2 text-[#0663fc] hover:underline" wire:click="copyParentLink({{ $participant->id }})">Link rodzica</button>
        ...
        <section class="client-portal-section space-y-3">
            <h3 class="text-sm font-semibold text-slate-900">{{ $editId ? 'Edycja' : 'Nowy uczestnik' }}</h3>
            <div>
                <label class="mb-1 block text-xs text-slate-600">Imię</label>
                <input type="text" wire:model="formFirstName" class="fi-input w-full rounded-lg border-slate-300 text-sm" />
            </div>
            <div>
                <label class="mb-1 block text-xs text-slate-600">Nazwisko</label>
                <input type="text" wire:model="formLastName" class="fi-input w-full rounded-lg border-slate-300 text-sm" />
            </div>
            <div>
                <label class="mb-1 block text-xs text-slate-600">Dieta</label>
            ...
            <div class="space-y-2 rounded-lg border border-slate-100 bg-slate-50 p-3">
                <p class="text-xs font-semibold uppercase tracking-wide text-slate-500">Zgody</p>
