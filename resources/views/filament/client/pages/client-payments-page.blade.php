    ...
    <div class="space-y-4">
        @if($isParticipant || ($isGuardian && $contract))
            <section class="client-portal-section overflow-hidden !p-0">
                    ...
                    <div class="flex flex-wrap items-start justify-between gap-3">
                        ...
                        @if(filled($statusLabel))
                            <span @class([
                                ...
                                'bg-slate-100 text-slate-700' => ! str_contains((string) $statusLabel, 'Opłac') && ! str_contains((string) $statusLabel, 'zęści'),
                            ])>
                                {{ $statusLabel }}
                            </span>
                        @endif
                ...
                @if(is_array($dietBreakdown) && ($dietBreakdown['enabled'] ?? false) && (($dietBreakdown['surcharge_pln'] ?? 0) > 0.009 || ($dietBreakdown['base_pln'] ?? 0) > 0.009))
                    <dl class="grid gap-px border-b border-slate-100 bg-slate-100 sm:grid-cols-3">
                        <div class="bg-white px-5 py-3">
                            <dt class="text-xs uppercase tracking-wide text-slate-500">Cena bazowa</dt>
                            <dd class="mt-1 text-sm font-semibold tabular-nums text-slate-900">
                                {{ MoneyFormatter::format((float) $dietBreakdown['base_pln'], 'PLN') }}
                            </dd>
                        </div>
                        <div class="bg-white px-5 py-3">
                            <dt class="text-xs uppercase tracking-wide text-slate-500">Dopłata diety</dt>
                            <dd class="mt-1 text-sm font-semibold tabular-nums text-[#0663fc]">
                                {{ MoneyFormatter::format((float) $dietBreakdown['surcharge_pln'], 'PLN') }}
                            </dd>
                        </div>
                        <div class="bg-white px-5 py-3">
                            <dt class="text-xs uppercase tracking-wide text-slate-500">Razem</dt>
                            <dd class="mt-1 text-sm font-semibold tabular-nums text-slate-900">
                ...
                @if($duePln !== null)
                    <dl class="grid gap-px bg-slate-100 sm:grid-cols-3">
                        <div class="bg-white px-5 py-4">
                            <dt class="text-xs uppercase tracking-wide text-slate-500">Należne PLN</dt>
                        ...
                        <div class="bg-white px-5 py-4">
                            <dt class="text-xs uppercase tracking-wide text-[#0663fc]">Różnica PLN</dt>
                    ...
                    <div class="flex flex-wrap items-center gap-3 border-t border-slate-100 px-5 py-4">
                        <div class="min-w-0 flex-1">
                            ...
                            @if(filled($chargeLabel))
                                <p class="text-xs text-slate-500">{{ $chargeLabel }}</p>
                            @endif
                        </div>
                        @if($chargeScheduleId)
                            <x-filament::button
                ...
                @elseif($remainingPln !== null && $remainingPln <= 0.009 && $duePln > 0.009)
                    <div class="border-t border-slate-100 px-5 py-3 text-sm text-emerald-700">
        ...
        @elseif($isGuardian && ! $isParticipant)
            <section class="client-portal-section">
                <h3 class="text-lg font-semibold text-slate-900">Płatności grupy</h3>
                <p class="mt-2 text-sm text-slate-600">
                    Jako opiekun widzisz harmonogram umowy grupowej.
                    @if($groupPaymentsUrl)
                        Szczegóły wpłat uczestników:
                        <a href="{{ $groupPaymentsUrl }}" class="font-medium text-[#0663fc] underline">Wpłaty grupy</a>.
                    @endif
                </p>
            </section>
        @endif
        ...
        @else
            <section class="client-portal-section overflow-hidden !p-0">
                <header class="flex flex-wrap items-center justify-between gap-2 border-b border-slate-100 px-5 py-4">
                    <div>
                        <h3 class="text-base font-semibold text-slate-900">Harmonogram</h3>
                ...
                @if(empty($presentation['payment_schedules']))
                    <p class="px-5 py-4 text-sm text-slate-500">Brak zdefiniowanych transz — obowiązuje kwota umowy.</p>
                @else
                                ...
                                @foreach($presentation['payment_schedules'] as $schedule)
                                    ...
                                    <tr @class(['bg-sky-50/70' => $canPayOnline])>
                                        <td class="px-5 py-3.5">
                                            <div class="font-medium text-slate-900">{{ $schedule['label'] ?: 'Transza' }}</div>
                                            @if ($isForeign)
                                                <div class="text-xs text-amber-700">Waluta · {{ $fxPlace }} · poza przelewem PLN</div>
                                            @endif
                                        </td>
                                        <td class="px-5 py-3.5 text-slate-600">{{ $schedule['due_date'] ?: '—' }}</td>
                                        ...
                                        <td class="px-5 py-3.5">
                                            ...
                                            @elseif($canPayOnline)
                                                <span class="font-semibold text-[#0663fc]">Teraz</span>
                                            @else
                                                <span class="text-slate-600">Oczekuje</span>
                                            @endif
                                        </td>
        ...
        @if($isGuardian && $groupPaymentsUrl && $isParticipant)
            <p class="text-sm text-slate-500">
                ...
                <a href="{{ $groupPaymentsUrl }}" class="font-medium text-[#0663fc] underline">zobacz wpłaty grupy</a>.
            </p>
        @endif
    </div>
</x-filament-panels::page>
