    ...
    @php
        ...
        if (! $isSetParent && ! $hideFinance) {
            if ($paidStatus === 'full') {
                ...
                if (($resolver['code'] ?? '') === 'N/A' || ($resolver['color'] ?? '') === 'gray') {
                    $payTone = 'danger';
                    $payLabel = 'Brak kwoty';
                } else {
                    $payTone = 'danger';
        ...
        $amountLabel = $hideFinance ? '—' : (
            ($planned !== '' && $planned !== '—') ? $planned : 'brak kwoty'
        );
    ...
    <div @class([
        ...
        <div class="epp-card__main">
            ...
            @if (is_array($remainingLine) && filled($remainingLine['text'] ?? null))
                <div class="epp-card__meta-line epp-card__meta-line--due">
                    {{ $remainingLine['text'] }}
        ...
        <div class="epp-card__side">
            ...
            @if ($doc)
                ...
                @endphp
                @if ($hasFile)
                    <a href="{{ $doc['url'] }}" target="_blank" rel="noopener noreferrer" class="epp-doc-icon epp-doc-icon--has" title="{{ $doc['title'] }}" x-on:click.stop>
                        <x-filament::icon icon="heroicon-m-document-text" class="epp-doc-icon__svg" />
                    </a>
                @elseif (($doc['hint'] ?? '') !== '' && ($doc['hint'] ?? '') !== 'Brak pliku')
                    <span class="epp-doc-icon epp-doc-icon--warn" title="{{ $doc['title'] }}">
                        <x-filament::icon icon="heroicon-m-document-text" class="epp-doc-icon__svg" />
                    </span>
                @else
                    <span class="epp-doc-icon epp-doc-icon--empty" title="Brak pliku">
                        <x-filament::icon icon="heroicon-m-document-text" class="epp-doc-icon__svg" />
                    </span>
                @endif
            @endif
