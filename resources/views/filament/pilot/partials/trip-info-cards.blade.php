        ...
        <div>
            <div class="portal-card">
                ...
                <div class="portal-info-grid">
                    ...
                    @php
                        $fleetVehicle = $event->relationLoaded('eventVehicles')
                            ? $event->mainFleetVehicle()
                            : $event->loadMissing(['eventVehicles.vehicle'])->mainFleetVehicle();
                    @endphp
                    @if($fleetVehicle?->manufacture_year)
                        <div>
                            <p class="label">Rocznik</p>
                            <p class="value">{{ $fleetVehicle->manufacture_year }}</p>
                        </div>
                    @endif
                    @if($fleetVehicle && (filled($fleetVehicle->brand) || filled($fleetVehicle->model)))
                        <div>
                            <p class="label">Marka / model</p>
                            <p class="value">{{ trim(implode(' ', array_filter([(string) $fleetVehicle->brand, (string) $fleetVehicle->model]))) }}</p>
                        </div>
                    @endif
                </div>
            </div>

            @if(\Illuminate\Support\Facades\Schema::hasTable('event_vehicles'))
                <div class="mt-3">
                    @livewire(\App\Livewire\PilotEventVehiclePhotos::class, ['event' => $event], key('pilot-vehicle-photos-'.$event->id))
                </div>
            @endif
