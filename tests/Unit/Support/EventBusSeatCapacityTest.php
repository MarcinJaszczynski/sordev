...
class EventBusSeatCapacityTest extends TestCase
{
    ...
    public function test_resolve_fleet_message_falls_back_to_event_main_vehicle(): void
    {
        $vehicle = Vehicle::factory()->create([
            'registration_number' => 'WW FALLBACK',
            'capacity' => 19,
        ]);
        $event = Event::factory()->create([
            'bus_id' => null,
        ...
        $message = EventBusSeatCapacity::resolveFleetMessage(
            fn (string $key) => null,
            $event->fresh(['eventVehicles.vehicle'])
        );

        $this->assertNotNull($message);
        $this->assertStringContainsString('WW FALLBACK', $message);
        $this->assertStringContainsString('Brakuje 1 miejsce', $message);
    }
}
