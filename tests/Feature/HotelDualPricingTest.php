...
class HotelDualPricingTest extends TestCase
{
    ...
    public function test_snapshot_seeds_offer_and_negotiated_from_catalog(): void
    {
        ...
        $this->assertEqualsWithDelta(400.0, (float) $line->offer_unit_price, 0.01);
        $this->assertSame(
    ...
    public function test_apply_negotiated_room_rates_updates_only_unit_price(): void
    {
        $event = $this->makeEventWithTwinLine(offer: 400, negotiated: 400);
        $service = app(EventHotelPlanService::class);
        ...
        $line = EventHotelRoomLine::query()->whereHas('stay', fn ($q) => $q->where('event_id', $event->id))->first();
        $this->assertEqualsWithDelta(360.0, (float) $line->unit_price, 0.01);
        $this->assertEqualsWithDelta(400.0, (float) $line->offer_unit_price, 0.01);
    }
    ...
    private function makeEventWithTwinLine(float $offer, float $negotiated): Event
    {
        ...
        return $event->fresh(['hotelStays.roomLines']);
    }
}
