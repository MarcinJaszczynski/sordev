...
class PilotPortalTest extends TestCase
{
    ...
    public function test_biuro_preview_can_open_dashboard_settlement_and_hotel_plan(): void
    {
        Role::firstOrCreate(['name' => 'biuro']);
        Role::firstOrCreate(['name' => 'super_admin']);

        $office = User::factory()->create(['status' => 'active']);
        ...
        $previewQuery = '?preview=1&pilot='.$pilot->id;

        $this->actingAs($office)
            ->get('/pilot'.$previewQuery)
            ->assertOk();

        $this->actingAs($office)
            ->get('/pilot/pilot-events/'.$event->id.$previewQuery)
            ->assertOk();

        $this->actingAs($office)
            ->get('/pilot/settlement/'.$event->id.$previewQuery)
            ->assertOk();

        $this->actingAs($office)
            ->get('/pilot/hotel-plan/'.$event->id.$previewQuery)
            ->assertOk();

        $this->assertTrue((bool) session('pilot_preview_mode'));
        $this->assertSame($pilot->id, (int) session('pilot_preview_user_id'));
    }
