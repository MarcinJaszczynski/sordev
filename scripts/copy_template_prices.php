...
$id = $argv[1] ?? null;
if (! $id) {
    echo "Usage: php copy_template_prices.php <event_template_id>\n";
    exit(1);
}
$ts = date('Ymd_His');
...
if (empty($common)) {
    echo "No common columns found for $table\n";
    exit(1);
}
...
// Fetch rows from src
$selCols = implode(', ', array_map(function ($c) {
    return "`$c`";
}, $common));
...
$total = count($rows);
if ($total === 0) {
    echo "No rows in source for event_template_id=$id\n";
    exit(0);
}
